...
public class ZoomOnSelect : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
{
    ...
    void SelectionTween()
    {
        ...
        /*
        transform.DORotate(new Vector3(0, 0, 25f), 0.1f, RotateMode.LocalAxisAdd).SetEase(Ease.InOutBounce).OnComplete
        (() => transform.DORotate(new Vector3(0, 0, -25f), 0.2f, RotateMode.LocalAxisAdd).SetEase(Ease.InOutBounce));*/
        #region uitleg lambda
        ...
        #endregion
        #region uitleg Sequence
        //DOTween Sequence. Met een sequence  kunnen we 
        // verschillende animaties stacken oftewel achter elkaar afspelen.
        //Nomaalgezien speelt DOTween animations die regel per regel staan tegelijk af als 1 combined animatie
    ...
    public void OnPointerEnter(PointerEventData pointerEventData)
    {
        ...
        audioSource.PlayOneShot(hoverSound[Random.Range(0,hoverSound.Length)]);
        transform.DOScale(startScale * scale, 0.15f);
    }

    public void OnPointerExit(PointerEventData pointerEventData)
    {
        // if (pointerEventData.pointerExit == gameObject)
